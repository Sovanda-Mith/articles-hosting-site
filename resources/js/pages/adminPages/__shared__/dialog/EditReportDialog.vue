<template>
  ...
  >
    ...
    >
      <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Report</h2>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        <div>
          ...
          >
            ...
            <option
              v-for="option in props.reporterOptions"
              :key="option.id"
              :value="option.id"
            >
              {{ option.name }}
...
import { reactive, watch, defineEmits, defineProps } from 'vue'
  ...
  watch(
    ...
    (r) => {
      if (r) {
        form.reporter_id = r.reporter_id
        form.problem = r.problem
        form.articleId = r.articleId
      }
    },
    { immediate: true }
  )
