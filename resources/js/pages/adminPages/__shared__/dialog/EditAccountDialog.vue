...
watch(
  ...
  (u) => {
    if (props.mode === 'edit' && u) {
      form.name = u.name || ''
      form.email = u.contact.email || ''
      form.occupation = u.contact.occupation || ''
      form.gender = u.gender || ''
      form.phone_number = u.contact.phone || ''
      form.address_line = u.address.line || ''
      form.city = u.address.city || ''
      form.country = u.address.country || ''
      form.dob = u.dateOfBirth ? u.dateOfBirth.slice(0, 10) : ''

      if (u.photo) {
        ;(async () => {
          form.pf_image = await convertImageUrlToBase64(u.photo!)
        })()
      } else {
        form.pf_image = ''
      }
